...
program
  ...
  .option(
    '--preset <names>',
    'Use preset tool categories (comma-separated). Available: mail, calendar, files, personal, work, excel, contacts, tasks, onenote, search, users, bookings, all'
  )
  .option('--list-presets', 'List all available presets and exit')
  .option(
    '--disabled-tools <pattern>',
    'Exclude tools matching regex pattern (e.g., "onenote|notebook" to disable OneNote tools)'
  )
  .option(
    '--disabled-presets <names>',
    'Exclude preset tool categories (comma-separated). Example: --disabled-presets onenote,tasks'
  )
  .option(
    '--org-mode',
...
export function parseArgs(): CommandOptions {
  ...
  // Handle disabled presets - convert to regex pattern
  if (options.disabledPresets) {
    const presetNames = options.disabledPresets.split(',').map((p: string) => p.trim());
    try {
      options.disabledTools = getCombinedPresetPattern(presetNames);
    } catch (error) {
      console.error(`Error: ${(error as Error).message}`);
      process.exit(1);
    }
  ...
  // Environment variable support for disabled tools
  if (process.env.DISABLED_TOOLS) {
    options.disabledTools = process.env.DISABLED_TOOLS;
  }

  if (process.env.DISABLED_PRESETS) {
    const presetNames = process.env.DISABLED_PRESETS.split(',').map((p: string) => p.trim());
    try {
      options.disabledTools = getCombinedPresetPattern(presetNames);
    } catch (error) {
      console.error(`Error: ${(error as Error).message}`);
