...
export function registerGraphTools(
  ...
  let disabledToolsRegex: RegExp | undefined;
  if (disabledToolsPattern) {
    try {
      disabledToolsRegex = new RegExp(disabledToolsPattern, 'i');
      logger.info(`Tool exclusion enabled with pattern: ${disabledToolsPattern}`);
    } catch {
      logger.error(`Invalid tool exclusion regex pattern: ${disabledToolsPattern}. Ignoring filter.`);
    }
  }
  ...
  for (const tool of api.endpoints) {
    ...
    if (disabledToolsRegex && disabledToolsRegex.test(tool.alias)) {
      logger.info(`Skipping tool ${tool.alias} - matches exclusion pattern`);
      skippedCount++;
      continue;
    }
